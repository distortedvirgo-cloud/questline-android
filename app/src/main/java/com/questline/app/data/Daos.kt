...
interface QuestDao {
    ...
    /** Все закрытые (DONE) квесты живым потоком — босс месяца */
    @Query("SELECT * FROM quests WHERE status = 'DONE' ORDER BY closedAtMillis DESC")
    fun observeDone(): Flow<List<Quest>>
...
interface CoinsLedgerDao {
    ...
    /** Одноразовые бонусы: есть ли уже запись с таким reason и refId (для BOSS_WIN — refId месяца год*100+месяц) */
    @Query("SELECT COUNT(*) FROM coins_ledger WHERE reason = :reason AND refId = :refId")
    suspend fun countByReasonRef(reason: String, refId: Long): Int
